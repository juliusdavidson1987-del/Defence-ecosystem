...
- **Ask the guide (v4.9.0):** a conversational assistant (floating "💬 Ask the guide" → chat panel)
  that helps users find the right door, understand how bodies relate, use the tool and draft an
  approach. Grounded in the map — the client sends the conversation + a retrieved org shortlist
  (`finderCandidates`) + a branch/category overview to the public, rate-limited `guide` Edge Function
  (`claude-opus-5` + `web_search`, `GUIDE_*` caps); the reply references orgs as `[[id]]` → clickable
  chips that `focusNode`. **Map-first**, wider-web fallback flagged unverified. Read-only, no data
  stored. Widget: `#guideBtn`/`#guide` + `guideSend`/`guideRender` in index.html.
